...
{
    public class ChainMail : ArmourBase
    {
        ...
        {
            // set local variables
            Io.ItemData.ItemName = "Chain mail";
            Io.ItemData.Title = "Chain mail";
            SetLocalVariable("shop_listing", "Chain mail");
            Io.ItemData.Price = 7000f;
            Io.ItemData.Weight = 30f;
            Io.ItemData.Equipitem.GetElementModifier(LabLordGlobals.EQUIP_ELEMENT_AC).Value = -4f;
            Io.ItemData.SetObjectType(EquipmentGlobals.OBJECT_TYPE_ARMOR);
            Script.Instance.AddToGroup(Io, "ARMORY");

            Io.ItemData.Description = "A type of armour consisting of small metal rings linked together in a pattern to form a mesh.";
            // apply modifiers
            /*
